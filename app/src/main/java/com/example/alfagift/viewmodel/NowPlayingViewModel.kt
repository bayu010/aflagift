...
class NowPlayingViewModel: ViewModel(), LifecycleObserver {
    val state = MutableLiveData<Resource<Any>>()
    val dataList = MutableLiveData<MovieResponse>()

    fun getNowPlayingMovie(language: String? = "en-US", page: Int? = 1){
        CoroutineScope(Dispatchers.IO).launch {
            state.postValue(Resource.loading())
            ApiService.apiInterface.getNowPlayingMovie(apiKey = ConstValue.API_KEY, language = language, page = page, region = null).enqueue(object: Callback<MovieResponse> {
